  ...
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 5; j++) {
      if (abs(i - 2) + abs(j - 2) > 2) {
        ...
        if (start_x < new_dist.x + (punch_size * 0.5) && 
        new_dist.x - (punch_size * 0.5) < start_x &&
        start_y < new_dist.y + (punch_size * 0.5) &&
        new_dist.y - (punch_size * 0.5) < start_y
        ) {
          alpha = 0.0;
        }
      }
