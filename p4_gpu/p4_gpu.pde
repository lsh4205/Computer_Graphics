...
void draw() {
  ...
  for (int i = 0; i < size_n; i++) {
    for (int j = 0; j < size_n; j++) {
      beginShape();
      texture(sheep_texture);
      vertex(i * size_squad, j * size_squad, 0, float(i)/size_n, float(j)/size_n);
      vertex((i+1) * size_squad, j * size_squad, 0, float(i+1)/size_n, float(j)/size_n);
      vertex((i+1) * size_squad, (j+1) * size_squad, 0, float(i+1)/size_n, float(j+1)/size_n);
      vertex((i) * size_squad, (j+1) * size_squad, 0, float(i)/size_n, float(j+1)/size_n);
      endShape();
    }
  }
  popMatrix();
